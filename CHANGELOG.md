...
- BENCHMARKS.md (issue #11): naive baseline results over the extraction,
  entity-resolution, relationship, and risk benchmarks, plus Mermaid visual
  demonstrations pulled straight from the pinned-seed corpora. Generated by
  `examples/generate_benchmarks_doc.py`, which `make baselines` checks for
  drift in CI.

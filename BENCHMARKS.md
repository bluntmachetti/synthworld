...
The fixture inventory is exactly two aggregate metric-only files:

| File | Cases | Dedicated discrimination | SHA-256 |
|---|---|---|---|
| `tests/fixtures/c08_v2/asteria/baseline-records.json` | exact, missing, fabricated, wrong action, extra, discarded | Each negative lowers its dedicated quality metric from `6/6` to `5/6`. | `df2e1b321677d44ab99b48103f8d2b856938332dd3601aa35c746611a67b3731` |
| `tests/fixtures/c08_v2/enterprise/baseline-records.json` | exact, missing, fabricated, wrong action, extra | Missing lowers completeness to `5/6`; fabricated, wrong action, and extra set their dedicated error rate to `1/7`. | `cf3424f7fe463d50fd77b07444cfba2cfa5c1820af1dac6a5126a5ae734b6787` |

The records contain no submission rows, observation/evidence/action IDs,
outcomes, evaluator, or truth. They are deterministic review evidence, not
leaderboard results and not generated by this document's baseline command.
Their reproduction tests have not run in this reconciliation.

The two public fixtures use binding handles and same-kind distractors so products
must correlate evidence rather than echo a unique kind or expected public ID.
Exact candidate inventories and committed digests are in

.PHONY: baselines benchmark-registry benchmark-registry-check capabilities capabilities-check ci examples explorer-assets-check install lint metrics package reference-lab schemas test typecheck
...
package:
	...
	$(UV) run --isolated --no-project --no-cache --with ./$(WHEEL) python -c "import tempfile; from pathlib import Path; from synthworld.cli import main; root=Path(tempfile.mkdtemp()); package=root/'asteria'; output=root/'world.html'; assert main(['generate-agentic','--output',str(package)]) == 0; assert main(['visualize','--public-package',str(package/'public'),'--view','agent-authority','--output',str(output)]) == 0; payload=output.read_bytes(); assert b'public projection only' in payload and b'id=\"synthworld-evaluator-overlay\"' not in payload"
...
ci: capabilities-check benchmark-registry-check lint typecheck package test metrics examples baselines schemas

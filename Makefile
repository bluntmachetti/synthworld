...
WHEEL := dist/idcognito_synthworld-0.9.0-py3-none-any.whl
...
package:
	...
	$(UV) run --isolated --no-project --with ./$(WHEEL) python -c "from synthworld.agentic import load_golden_agentic_benchmark; assert len(load_golden_agentic_benchmark().public.events) == 24"
